...
public class SectionExecutionFactory
{
    ...
    static void setExpectedPartitionsForFilters(DynamicFilterService dynamicFilterService, QueryId queryId, PlanNode root, int taskCount)
    {
        ...
        // safe because each JoinDynamicFilter belongs to exactly one join node in one fragment, and
        // the three call sites of this method are in mutually exclusive scheduling branches
        // (SOURCE_DISTRIBUTION → deferred via state-change listener, SINGLE_DISTRIBUTION else-if,
        // and fixed-partitioned else-if). A given fragment enters exactly one branch, so each
        // filter's setExpectedPartitions() is called at most once.
